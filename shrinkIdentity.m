...
function [sigma,shrinkage]=shrinkIdentity(x,shrink)
...
sample=(1/t).*(x'*x);
% compute prior
% here we replace the shrinkage target to the Identity matrix of the right
% dimensions.
prior=eye(length(sample)); % since sample is a n*n-matrix
% as before
var=diag(sample);
sqrtvar=sqrt(var);
